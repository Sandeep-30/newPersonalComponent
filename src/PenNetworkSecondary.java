...
public abstract class PenNetworkSecondary implements PenNetwork {
    ...
    /**
     * Helper method to turn a PenSKU into a String key, since the current
     * kernel only supports String-based pen identifiers.
     */
    ...
    @Override
    public final boolean requestTransfer(String fromId, String toId, PenSKU sku,
            int qty) {
        if (fromId == null || toId == null || sku == null || qty <= 0) {
            return false;
        }
        String penKey = this.key(sku);
    ...
    public Optional<TransferPlan> computeRestockPlan(String needyShopId,
        ...
        return Optional
                .of(new TransferPlan(bestDonor, needyShopId, sku, transferQty));
    }
